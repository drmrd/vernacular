...
export abstract class SurfaceMaterialProvider implements MaterialProvider {
  ...
  /**
   * A painted surface rendered as a standard material: the treatment tint plus any
   * pattern fields. Both a plain paint and a physical provider's non-solid treatment
   * resolve to this, so the standard-material construction lives in one place.
   */
  protected standardPaintedMaterial(
    role: SurfaceRole,
    treatment: SurfaceTreatment,
  ): THREE.Material {
