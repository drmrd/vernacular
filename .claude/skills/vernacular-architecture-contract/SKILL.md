...
## Known-weak points, stated plainly (as of 2026-07-05; item 4 updated 2026-08-14)
...
4. **`finishId` is read in `engine/` now; this weak point closed on 2026-07-14.** A solid `SurfaceTreatment` carries `finishId` (`core/model/paint.ts`), and `engine/materials/physical-material-provider.ts` resolves it against the finishes registry: a solid paint renders as a `MeshPhysicalMaterial` whose roughness, sheen, and specular come from the registry entry, and an unregistered finish falls back to the matte entry so it never inherits three.js's glossy material defaults (ADR-0067, ADR-0130). `PhysicalMaterialProvider` is constructed at both live-view build entry points, `bridge/react/framed-scene.ts` and `bridge/react/floor-build.ts`. The color-only `PaintMaterialProvider` this item once named was deleted on 2026-08-14 (#513), so do not cite `engine/materials/paint-material-provider.ts`. The registry-aware `pattern` lookup for wall and cladding materials is still reserved for the interior-wall slice (ADR-0130).
...
- finishId resolved in engine: `grep -rn finishId engine/materials/physical-material-provider.ts` (expect hits; empty means the finish wiring regressed)
